...
int servostop_R = 1500;//set initial servo position: Right
int servostop_L = 1500;//set initial servo position: Left
int servoadjust_R = 0;//set additional speed to servo (0 to 255): Right 
int servoadjust_L = 0;//set additional speed to servo (0 to 255): Left
...
if(direction_r==1){SERVO_R.writeMicroseconds(servostop_R-speednow-servoadjust_R);}
else{SERVO_R.writeMicroseconds(servostop_R+speednow+servoadjust_R);}
if(direction_l==1){SERVO_L.writeMicroseconds(servostop_L+speednow+servoadjust_L);}
else{SERVO_L.writeMicroseconds(servostop_L-speednow-servoadjust_L);}
...
void move_stop(){  SERVO_R.writeMicroseconds(servostop_R);SERVO_L.writeMicroseconds(servostop_L); SERVO_R.detach();SERVO_L.detach();moveflag=false;speednow=0;} //detach pins for reduce malfunction, but before detach, need to write servostop for next smoothly moving
void move_brakes(){SERVO_R.writeMicroseconds(servostop_R);SERVO_L.writeMicroseconds(servostop_L); SERVO_R.detach();SERVO_L.detach();moveflag=false;speednow=0;}
